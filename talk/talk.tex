...
\begin{frame}{Beyond the single best model}
  \begin{itemize}
    ...
    \item For a tolerance $\varepsilon > 0$, the \emph{Rashomon set}
      ...
      {\footnotesize Notation: $\fhDlam$ = model trained on data $\D$ with hyperparameter configuration $\lamv \in \Lam$; $\lamv^{\ast}$ = risk-minimising configuration; $\risk(\cdot)$ = (empirical) risk functional.}
...
\begin{frame}{Pipeline}
  \begin{enumerate}
    ...
    \item Build the pairwise \emph{behavioural distance} matrix $\mathbf{D}$ with entries
      ...
      \]
      where $i, j$ index models in $\Rset$ and $o$ runs over the $n_{\text{val}}$ validation observations (see backup slide).
  ...
  \end{enumerate}
  \vspace{0.5ex}
  {\footnotesize Predictions are class probabilities for classification, $\fh(\xv)$ for regression. Distances computed for both Euclidean ($p = 2$) and Manhattan ($p = 1$); results shown are Euclidean (Manhattan tells the same story).}
\end{frame}
