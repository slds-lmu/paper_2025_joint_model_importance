...
for (taskname in names(preds_by_task)) {
  ...
  for (learner in unique(td$learner)) {
    ...
    if (n_fam < min_family_size) {
      message(taskname, " / ", learner, ": only ", n_fam,
    ...
    hp_task <- hp_table[taskname == ..taskname]

    if (nrow(hp_task) < max(fam_model_no)) {
      message(taskname, " / ", learner, ": HP table has only ",
    ...
    message(taskname, " / ", learner, ": plotting ", length(hp_cols),
            " HP(s) for ", n_fam, " models")

    n_per_row <- min(3, length(hp_cols))
    n_panel_rows <- ceiling(length(hp_cols) / n_per_row)
    pdf(file.path(hp_dir, sprintf("hp_vs_mds1_%s_%s.pdf", taskname, learner)),
    ...
    for (hp in hp_cols) {
      ...
      if (is.factor(val) || is.character(val) || is.logical(val)) {
        plot(factor(val), fam_mds1,
             ...
             main = sprintf("%s / %s", taskname, learner))
      } else {
        plot(val, fam_mds1,
             xlab = hp, ylab = "MDS 1 (within-family)",
             main = sprintf("%s / %s", taskname, learner),
